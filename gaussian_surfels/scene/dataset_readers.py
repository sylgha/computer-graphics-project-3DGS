...
def readIDRCameras(path):
    ...
    for i in range(n_images):
        ...
        # Monocular priors: normal + (optional) depth maps.
        # We support multiple depth sources in the following priority:
        #   1) DA3 relative depth:  depth/{image_name}_rel_depth.npy
        #   2) DA3 metric depth:    depth/{image_name}_metric_depth.npy
        #   3) Legacy depth prior:  depth/{image_name}_depth.npy
        # All available depth maps are concatenated after the 3-channel normal prior.

        # Normal prior (may be missing for some datasets, e.g. HOPE video).
        try:
        ...
        # DA3 relative depth (resize to match original image resolution)
        try:
            da3_rel = read_monoData(f'{path}/depth/{image_name}_rel_depth.npy')
            da3_rel = resize_image(da3_rel, [image.shape[0], image.shape[1]])
            depth_channels.append(da3_rel)
        except FileNotFoundError:
            da3_rel = None

        # DA3 metric depth (in meters, resize to match original image resolution)
        try:
            da3_metric = read_monoData(f'{path}/depth/{image_name}_metric_depth.npy')
            da3_metric = resize_image(da3_metric, [image.shape[0], image.shape[1]])
            depth_channels.append(da3_metric)
        except FileNotFoundError:
            da3_metric = None

        # Legacy single-channel depth prior, used only if no DA3 depth exists.
        if len(depth_channels) == 0:
            try:
                legacy_depth = read_monoData(f'{path}/depth/{image_name}_depth.npy')
                legacy_depth = resize_image(legacy_depth, [image.shape[0], image.shape[1]])
                depth_channels.append(legacy_depth)
            except FileNotFoundError:
                legacy_depth = None

        if monoN is None and len(depth_channels) == 0:
            mono = None
        else:
            if monoN is None:
                mono_list = depth_channels
            elif len(depth_channels) == 0:
                mono_list = [monoN]
            else:
                mono_list = [monoN] + depth_channels
            mono = np.concatenate(mono_list, 0)
...
def readIDRSceneInfo(path, eval, idr_half_split=False, testskip=8):
    ...
    if eval:
        ...
        if idr_half_split:
            # 可选：将相机 1:1 划分为 train/test
            num_cams = len(cam_infos)
            mid = num_cams // 2
            train_cam_infos = cam_infos[:mid]
            test_cam_infos = cam_infos[mid:]
        else:
            # 默认行为：全部相机作为训练集，不单独划分测试集
            train_cam_infos = cam_infos
            test_cam_infos = []
