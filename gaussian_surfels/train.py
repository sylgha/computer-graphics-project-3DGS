...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 2):
        ...
        if mono is not None:
            mono *= mask_gt
            # If the first 3 channels exist, they are treated as normal prior.
            if mono.shape[0] >= 3:
                monoN = mono[:3]
                depth_ch = mono[3:]
            else:
                # No normal prior available; all channels are depth priors.
                depth_ch = mono

            # By convention in dataset_readers for depth_ch ordering:
            #   depth_ch[0] -> DA3 relative depth (or legacy depth if DA3 is absent)
            #   depth_ch[1] -> DA3 metric depth (if available)
            if depth_ch.shape[0] >= 1:
                mono_relD = depth_ch[:1]
            if depth_ch.shape[0] >= 2:
                mono_metricD = depth_ch[1:2]
        ...
        if monoN is not None:
            loss_monoN = cos_loss(normal, monoN, weight=mask_gt)
            # legacy depth supervision (not used by default)

        loss_surface = cos_loss(normal, d2n)

        # Optional depth supervision from DA3 priors
        loss_rel_depth = None
        loss_metric_depth = None
        if mono_relD is not None and opt.lambda_rel_depth > 0.0:
            # Scale-align relative depth prior to current prediction
            mono_rel_match, rel_mask = match_depth(
                mono_relD,
                depth,
                mask_gt * mask_vis,
                256,
            ...
            loss_rel_depth = l1_loss(depth * rel_mask, mono_rel_match)

        if mono_metricD is not None and opt.lambda_metric_depth > 0.0:
            # Direct L1 on metric depth (measured in meters)
            metric_mask = mask_gt * mask_vis
            loss_metric_depth = l1_loss(depth * metric_mask, mono_metricD * metric_mask)
        ...
        if loss_rel_depth is not None:
            loss += opt.lambda_rel_depth * loss_rel_depth

        if loss_metric_depth is not None:
            loss += opt.lambda_metric_depth * loss_metric_depth
    ...
    # 训练结束后记录总耗时（秒），方便整体对比不同实验
    try:
        total_time = time.time() - total_start_time
        time_log_path = os.path.join(dataset.model_path, "train_time.txt")
        with open(time_log_path, 'a') as f:
            f.write(f"iterations {opt.iterations} total_seconds {total_time}\n")
    except Exception:
        pass
...
def training_report(tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene : Scene, pipe, bg, use_mask):
    ...
    # 记录每次迭代的训练 loss，便于后续画曲线
    try:
        loss_log_path = os.path.join(scene.model_path, "losses.txt")
        with open(loss_log_path, 'a') as f:
            f.write(f"{iteration} {Ll1.item()} {loss.item()}\n")
    except Exception:
        pass

    # 额外把每次迭代用时记录到文本文件，方便在没有 TensorBoard 的环境下查看
    try:
        log_path = os.path.join(scene.model_path, "iter_times.txt")
        with open(log_path, 'a') as f:
            f.write(f"{iteration} {elapsed}\n")
    except Exception:
        pass
    ...
    if iteration in testing_iterations:
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                # 记录评估阶段的 L1 和 PSNR（按 train/test 分开存）
                try:
                    psnr_log_path = os.path.join(scene.model_path, f"psnr_{config['name']}.txt")
                    with open(psnr_log_path, 'a') as f:
                        f.write(f"{iteration} {float(l1_test)} {float(psnr_test)}\n")
                except Exception:
                    pass
                if tb_writer:
