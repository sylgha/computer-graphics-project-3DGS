...
class ModelParams(ParamGroup): 
    def __init__(self, parser, sentinel=False):
        ...
        # IDR 数据集是否在非 eval 模式下 1:1 划分 train/test
        self.idr_half_split = False
...
class OptimizationParams(ParamGroup):
    def __init__(self, parser):
        ...
        # Additional depth supervision weights (DA3 priors)
        # lambda_rel_depth: loss on relative depth (scale-aligned by match_depth)
        # lambda_metric_depth: loss on metric depth (direct L1 in meters)
        self.lambda_rel_depth = 0.0
        self.lambda_metric_depth = 0.0
